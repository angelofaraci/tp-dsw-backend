...
//verifies inputs
function sanitizeInteractionInput(
  req: Request,
  res: Response,
  next: NextFunction
) {
...
async function add(req: Request, res: Response) {
  ...
  return res
    .status(201)
    .send({ message: "Interaction created", data: newInteraction });
}
...
async function update(req: Request, res: Response) {
  ...
  const input = req.body.sanitizedInput;
  const interaction = await repository.findOneAndUpdate(
    { id: input.id },
    input
  );
  if (!interaction) {
  ...
  return res
    .status(200)
    .send({ message: "Interaction updated successfully", data: interaction });
}
